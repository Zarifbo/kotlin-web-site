...
values and inserts (safe) casts automatically when necessary:
...
From Kotlin 2.0.0, if you combine type checks for objects with an `or` operator (`||`), a smart cast
is made to their closest common supertype:
...
From Kotlin 2.0.0, the compiler can smart-cast variables captured within lambda functions that are passed to [inline functions](inline-functions.md).
...
From Kotlin 2.0.0, smart cast information is passed on to `catch` and `finally` blocks. This change makes your code safer
as the compiler keeps track of whether your object has a nullable type. For example:
...
To explicitly cast an object to a nullable type, use the *safe* cast operator `as?`:

```kotlin
val x: String? = y as? String
```

If the cast isn't possible, the compiler returns `null` on failure.
